...
test('PUT /api/data refuses a state whose workouts or routines is not an array', async t => {
  ...
  let revision = '"0"';
  let key = 0;
  const put = state => fetch(`${h.api}/api/data`, {
    method: 'PUT',
    headers: { ...cookie, 'If-Match': revision, 'Idempotency-Key': `shape-${++key}` },
  ...
  let r = await put({ _ts: 2, workouts: [], routines: [] });
  assert.equal(r.status, 200);
  revision = r.headers.get('etag');
  r = await put({ _ts: 3 });
  assert.equal(r.status, 200);
  revision = r.headers.get('etag');
  r = await put({ _ts: 4, workouts: null, routines: null });
  assert.equal(r.status, 200);
  revision = r.headers.get('etag');
