...
import { appendOneRm, canonicalJSON, defaultPlanRule, planWarmupRows, supports, validatePlanRule, validateWarmup, warmupMaxCount, warmupSteps } from './lib/prescription/index.js'
...
export function swapActiveWorkoutExercise(index) {
  ...
  const picker = exercisePicker((ex, quick) => {
    if (!quick) { exConfigSheet(ex, null, cfg => swapTo(ex, cfg), null, slotRoutine); return }
...
function ExConfig({ ex, existing, onSave, onDelete, close, routine }) {
  ...
  const save = () => {
    ...
    onSave({
      ...
      ...(!cardio && intensifier?.type ? { intensifier } : {}),
  ...
  return <>
    ...
    {cardio ? <div className="row cfgrow" style={{ marginBottom: 18 }}>
      ...
      <Stepper label={t('Speed (km/h)')} value={cardioCfg.speed} step={0.5} onChange={v => setCardioCfg(x => ({ ...x, speed: v }))} />
    ...
    {!cardio && <>
      ...
      {(can.dropset || can.restpause) && <Disclosure title={t('Intensifier')} value={intensifier?.type === 'dropset' ? t('Drop-set') : intensifier?.type === 'restpause' ? t('Rest-pause') : t('None')}>
        ...
        {intensifier?.type === 'dropset' && <div className="row cfgrow" style={{ marginTop: 8, marginBottom: 8 }}>
          <Stepper label={t('Drops')} value={intensifier.count} step={1} decimal={false}
            onChange={v => setIntensifier(x => ({ ...x, count: Math.min(5, Math.max(1, v)) }))} />
          <Stepper label={t('Weight drop (%)')} value={intensifier.pct} step={5} decimal={false}
            onChange={v => setIntensifier(x => ({ ...x, pct: Math.min(90, Math.max(5, v)) }))} />
        </div>}
        {intensifier?.type === 'restpause' && <div className="row cfgrow" style={{ marginTop: 8, marginBottom: 8 }}>
          <Stepper label={t('Rest-pause reps')} value={intensifier.totalReps} step={1} decimal={false}
            onChange={v => setIntensifier(x => ({ ...x, totalReps: Math.min(100, Math.max(1, v)) }))} />
          <Stepper label={t('Rest (s)')} value={intensifier.restSec} step={5} decimal={false}
            onChange={v => setIntensifier(x => ({ ...x, restSec: Math.min(120, Math.max(5, v)) }))} />
        </div>}
      </Disclosure>}
      {usesBar(ex) && <Disclosure title={t('Bar weight')} value={(barWeightFor(st, ex) || 0) + ' ' + st.unit}>
        <BarWeightEditor ex={ex} extra={t('Applies to this exercise everywhere, not just this plan.')} />
      </Disclosure>}
    </>}
    ...
    <div style={{ height: 8 }} />
    <Button variant="primary" disabled={!valid} onClick={save}>{existing ? t('Save') : t('Add to routine')}</Button>
...
export const exConfigSheet = (ex, existing, onSave, onDelete, routine, initial) => ui().openSheet(close => <ExConfig ex={ex} existing={existing} onSave={onSave} onDelete={onDelete} routine={routine} close={close} />)
...
function WorkoutDetail({ w, close }) {
  ...
  const st = useStore(s => s.S)
  const update = useStore(s => s.update)
  const entries = (w.exposures || []).map(exposure => {
    ...
    const sets = work.map(row => ({ done: true, r: row.observations?.find(item => item.metric === 'repetitions')?.value, sec: row.observations?.find(item => item.metric === 'duration')?.value, w: row.resistance?.kind === 'external-load' ? row.resistance.value : 0 }))
    return { id: exposure.exerciseId, n: exposure.exerciseNameSnapshot || exposure.muscleSnapshot?.n, rid: exposure.routineId, note: exposure.note, notePin: exposure.notePin, target, sets, volume: volumeOf({ sets: work }), exposure, prescription }
  })
  ...
  const entryRow = (e, i) => {
    ...
    return <div key={i} className="row" style={{ marginBottom: 12, alignItems: 'flex-start' }}>
      {ex && <Thumb ex={ex} />}
      <div className="grow"><div className="tt capitalize" style={{ fontWeight: 600 }}>{ex ? exerciseNameFor(ex) : (e.n || e.id)} {w.prs && w.prs.includes(e.id) && <span className="pr"><Icon name="trophy" />PR</span>} {badge && <span className="tag">{historyBadgeLabel(badge)}</span>}</div>
  ...
  return <>
    ...
    {grouped ? groups.map(g => {
      ...
      const setN = g.items.reduce((n, [e]) => n + e.sets.length, 0)
      const vol = g.items.reduce((n, [e]) => n + e.volume, 0)
...
function beginBackfill({ iso, time, durationMin, routineId, replaceId }) {
  ...
  useStore.getState().update(s => {
    const out = buildCombinedExposures(s, routineId ? [routineId] : [], { now: session.start, newId: uid, unit: s.unit })
    session.routineIds = out.routineIds
    session.name = out.routines.length ? deriveSessionName(out.routines.map(r => r.name)) : t('Freestyle')
    session.exposures = out.exposures
    session.entries = entriesForExposures(out.exposures, s.prescriptions)
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    <div className="tiles" style={{ textAlign: 'left' }}>
      ...
      <div className="tile"><div className="l">{t('Sets')}</div><div className="v" style={{ fontSize: '1.1rem' }}>{t('{0} sets · {1} work', setsDone(w), workSetsDone(st, w))}</div></div>
