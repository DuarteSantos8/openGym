...
describe('durable local persistence failures', () => {
  ...
  it('keeps a visible edit and marks it dirty when localStorage is full', async () => {
    ...
    console.log('gate2_localstorage_quota_edit_visible=true')
    console.log('gate2_localstorage_quota_error=local_storage_write_failed')
    console.log('gate2_localstorage_quota_dirty=true')
  ...
  it('keeps the edit and exposes native mirror failure after the debounce', async () => {
    ...
    expect(localStorage.getItem('gym_dirty')).toBe('1')
    console.log('gate2_native_persistence_edit_visible=true')
    console.log('gate2_native_persistence_error=native_persistence_failed')
    console.log('gate2_native_persistence_dirty=true')
  })
})
